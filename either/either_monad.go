...
type EitherMonad[L, R, T any] struct {
	e Either[L, R]
}

func (em *EitherMonad[L, R, T]) Ap(ff Either[L, func(value R) T]) Either[L, T] {
	if ff.IsRight() {
		return em.Map(ff.Right())
	}
	return Left[L, T](ff.Left())
}

func (em *EitherMonad[L, R, T]) FlatMap(mapper func(value R) Either[L, T]) Either[L, T] {
	if em.e.IsRight() {
		return mapper(em.Right())
	}
	return Left[L, T]((em.Left()))
}

func (em *EitherMonad[L, R, T]) Map(mapper func(value R) T) Either[L, T] {
	if em.e.IsRight() {
		return Right[L, T](mapper(em.Right()))
	}
	return Left[L, T]((em.Left()))
}

// func (em *EitherMonad[L, R, T]) ToOptionWrapper() option.OptionWrapper[R, T] {
// 	if em.IsRight() {
// 		return option.OptionWrapper[R, T]{
// 			o: option.Some(em.Right()),
// 		}
// 	}
// 	return option.OptionWrapper[R, T]{
// 			o: option.None(),
...
// func (em *EitherMonad[L, R, T]) MapLeft(mapper func(L) L) Either[L, T] {
// 	if em.e.IsRight() {
// 		return Right[L, T](em.Right()))
// 	}
// 	return em.LeftMap(em.Left())
// }

func (em *EitherMonad[L, R, T]) Left() L {
	if !em.e.IsLeft() {
...
func (em *EitherMonad[L, R, T]) Right() R {
	if !em.e.IsRight() {
...
func (em *EitherMonad[L, R, T]) IsLeft() bool {
	return !em.e.IsRight()
}

func (em *EitherMonad[L, R, T]) IsRight() bool {
	return em.e.IsRight()
}
...
// Left returns left value of a Either struct or panics.
...
// Right returns right value of a Either struct or panics.
...
func (e Either[L, R]) Map(mapper func(R) Either[L, R]) Either[L, R] {
	if e.isLeft {
...
func EitherMonadFromEither[L, R, T any](either Either[L, R]) EitherMonad[L, R, T] {
	return EitherMonad[L, R, T]{
		e: either,
	}
}
